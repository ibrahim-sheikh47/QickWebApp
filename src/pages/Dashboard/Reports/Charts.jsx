...
const BarChart = ({ data, selectedBar, setSelectedBar }) => {
    const maxHeight = Math.max(...data.map(item => item.data));


    return (
        <div className="flex w-full justify-between items-end relative mt-20">
            {data.map((value, index) => (
                <div key={index} className="flex-1 flex flex-col items-center relative font-PJSmedium text-sm">
                    <div
                        className={`w-5 rounded-md shadow cursor-pointer  ${selectedBar === value.label ? 'bg-lime' : 'bg-secondaryTen'
                            }`}
...
const BookingBar = () => {
    ...
    const barChartData = [
        { label: 'East Field', data: 30 },
        { label: 'Jr Field', data: 45 },
        { label: 'North Field', data: 20 },
        { label: 'South Field', data: 10 },
    ];
